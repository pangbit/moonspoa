...
MoonBit 实现的 SPOA(Stream Processing Offload Agent)库:HAProxy SPOE v1.2 的 SPOP 协议编解码、agent 服务器(TCP / Unix domain socket)与 SPOE 客户端。目标后端:**native**。

> 模块名当前为 `yuebingo/spoa`,发布前请改成自己的模块名(`moon.mod` 的 `name` 字段及各 `moon.pkg` 的 import 路径)。
...
```bash
moon add yuebingo/spoa   # 发布后;当前请使用本地路径或自行改名
```
...
async fn main {
  let agent = @agent.Agent::new().on("check-ip", fn(msg) {
    ...
    [SetVar(Transaction, "txn.ip_score", Int32(85))]
  })
...
frontend http-in
    bind *:8080
    filter spoe engine ipscore conf /etc/haproxy/spoe-ipscore.conf
...
HAProxy 会把 `src` 作为 `ip` 参数发送 `check-ip` 消息;agent 回的 `set-var` 动作(带 `ipscore.` 前缀)可在后续规则中使用,例如 `http-request deny if { var(ipscore.txn.ip_score) -m int gt 80 }`。
